...
EOD;
		...
		public function generate_responsive_styles( $options = array() ) {
			...
			if ( 
				! empty( $options ) and 
				( isset( $options['responsive_columns'] ) and (bool)$options['responsive_columns'] ) and 
			...
			$this->sections[] = array(
				...
				'fields' => array(
					...
					array(
						...
						'desc' => __( 'The title appears just and after the image counter and before the caption in the slideshow.', 'rps-image-gallery' ),
						'default' => true,
						'required' => array( 'fb_title_show', 'equals', true ),
						),
					array(
			...
			$this->sections[] = array(
				...
				'fields' => array(
					array(
						...
						'desc' => __( 'Load default plugin styles.', 'rps-image-gallery' ),
						'default' => true,
						),
					array(
						'id'=>'load_fancybox',
						...
						'desc' => __( 'Load included fancyBox script and assets.', 'rps-image-gallery' ),
						'default' => true,
						),
					array(
		...
		public function _init() {
			...
			wp_register_script( 'rps-image-gallery-pagination', plugins_url( 'dependencies/pagination.js', __FILE__ ), array( 'jquery' ), '1.0.0', true );
		
			switch ( $fb_version ) {
				case '2':
					wp_register_style( 'rps-image-gallery-fancybox', plugins_url( 'dependencies/fancybox2/jquery.fancybox.css?v=2.1.5', __FILE__ ), false, '2.1.5' );
					wp_register_script( 'rps-image-gallery-fancybox', plugins_url( 'dependencies/fancybox2/jquery.fancybox.pack.js?v=2.1.5', __FILE__ ), array( 'jquery' ), '2.1.5', true );
					break;
				default:
		...
		public function _enqueue_styles_scripts() {
			...
			if ( 
				...
				( isset( $_rps_image_gallery['load_styles'] ) and (bool)$_rps_image_gallery['load_styles'] ) 
			) {
				
				wp_enqueue_style( 'rps-image-gallery' );
								
				if ( 
					( isset( $_rps_image_gallery['responsive_columns'] ) and (bool)$_rps_image_gallery['responsive_columns'] ) and 
					( 
					...
					if ( ! isset( $_rps_image_gallery['load_responsive_styles_as'] ) or ( isset( $_rps_image_gallery['load_responsive_styles_as'] ) and $_rps_image_gallery['load_responsive_styles_as'] === 'link' ) ) {
					...
					elseif ( isset( $_rps_image_gallery['load_responsive_styles_as'] ) and $_rps_image_gallery['load_responsive_styles_as'] === 'inline' ) {
				...
				switch ( true ) {
					...
					case ( ! isset( $_rps_image_gallery['theme'] ) or ( isset( $_rps_image_gallery['theme'] ) and $_rps_image_gallery['theme'] === 'default' ) ) :
			...
			if ( 
				( isset( $_rps_image_gallery['facebook_enable'] ) and $_rps_image_gallery['facebook_enable'] ) or 
				( isset( $_rps_image_gallery['pinterest_enable'] ) and $_rps_image_gallery['pinterest_enable'] ) 
			) {
		...
		public function cb_gallery_shortcode( $atts, $content = null ) {
			...
			foreach ( $attachments as $key => $attachment ) {
				...
				if ( isset( $image_meta['height'], $image_meta['width'] ) ) {
					$image->setOrientation( ( $image_meta['height'] > $image_meta['width'] ) ? 'portrait' : 'landscape' );
				}
		...
		private function generate_title_value( $attachment, $caption, $display_heading, $display_caption, $display_exif, $exif_locations, $exif_fields, $alt_caption_fallback ) {
			...
			$title_parts['exif'] = ( $display_exif and ( 'slideshow' === $exif_locations or 'both' === $exif_locations ) ) ? self::generate_slideshow_exif ( $attachment->ID, $exif_fields ) : '';
