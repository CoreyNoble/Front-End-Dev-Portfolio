...
function nsg_show() 
{
	...
	$nsg_pluginurl = plugins_url() . "/new-simple-gallery/";
...
function nsg_admin_option()
{
	...
	<div class="wrap">
	  <div class="form-wrap">
		...
		{
			$nsg_xml_file = stripslashes(sanitize_text_field($_POST['nsg_xml_file']));
			$nsg_random = stripslashes(sanitize_text_field($_POST['nsg_random']));
			$nsg_title = stripslashes(sanitize_text_field($_POST['nsg_title']));
			//$nsg_dir = stripslashes(sanitize_text_field($_POST['nsg_dir']));
			$nsg_width = stripslashes(intval($_POST['nsg_width']));
			$nsg_height = stripslashes(intval($_POST['nsg_height']));
			$nsg_pause = stripslashes(intval($_POST['nsg_pause']));
			$nsg_duration = stripslashes(intval($_POST['nsg_duration']));
			$nsg_cycles = stripslashes(intval($_POST['nsg_cycles']));
			
			if(!is_numeric($nsg_width) || $nsg_width == 0) { $nsg_width = 200; }
			if(!is_numeric($nsg_height) || $nsg_height == 0) { $nsg_height = 200; }
			if(!is_numeric($nsg_pause) || $nsg_pause == 0) { $nsg_pause = 2500; }
			if(!is_numeric($nsg_duration) || $nsg_duration == 0) { $nsg_duration = 500; }
			if(!is_numeric($nsg_cycles) || $nsg_cycles == 0) { $nsg_cycles = 2; }
